...
class PertData:
    ...
    def __init__(self, data_path, gi_go = False, gene_path = None):
        ...
        self.pert_names = np.unique(list(gene2go.keys())) #67832  gene
        """
    ...
    def load(self, data_name = None, 
        ...
        if data_name in ['norman', 'adamson', 'dixit']:
            ...
            adata_path = os.path.join(data_path, 'norman.h5ad')
        ...
        ######load这个分支
            ...
            print(data_path)
            adata_path = os.path.join(data_path, 'norman.h5ad') #norman   #/home/lxz/scmamba/gene_pretubation/GEARS/data     传进来的data_path 
        ...
        ##找出数据集中所有“无效”的扰动条件，并将它们的名称收集到一个列表中。一个条件被视为“无效”​当且仅当它包含至少一个既不是 'ctrl' 也不在已知基因列表 self.pert_names 中的“未知基因”。
        print("无效扰动")
        print_sys(not_in_go_pert) ###[]
        #创建一个过滤器，用于选中数据集中所有“有效”的细胞。​遍历 self.adata.obs['condition'] 列,如果 condition == 'ctrl',如果 condition 是有效的双基因扰动(ctrl+ in pert_name)​都返回true
        ...
        pyg_path = os.path.join(data_path, 'geneformer/data_pyg') 
    ...
    def new_data_process(self, dataset_name,
        ...
        self.ctrl_adata = self.adata[self.adata.obs['condition'] == 'ctrl']
        self.gene_names = self.adata.var.gene_name
        pyg_path = os.path.join(save_data_folder, 'geneformer/data_pyg') 
        if not os.path.exists(pyg_path):
            os.mkdir(pyg_path)
        dataset_fname = os.path.join(pyg_path, 'cell_graphs.pkl')
        print_sys("Creating pyg object for each cell in the data...")
    ...
    def prepare_split(self, split = 'simulation', 
        ...
        split_folder = os.path.join(self.dataset_path, 'geneformer/splits') 
    ...
    def get_dataloader(self, batch_size, test_batch_size = None):
        ...
        # if 'HMGA2' in self.adata.var.gene_name:
        #     print("HMGA2 存在!!!")
        # Create cell graphs
        cell_graphs = {}
