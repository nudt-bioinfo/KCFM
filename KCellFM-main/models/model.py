...
class BiMambaWrapper(nn.Module):
    ...
    def __init__( 
            ...
            bidirectional_weight_tie: bool = True, #for  和  rev需要一样吗?应该是不用的?
    ...
    #         # Reverse sequence dimension
    #         reversed_states = hidden_states.flip(dims=(1,))
...
class MambaModel(nn.Module):
    ...
    #     total_embs = src + values #这里基因和值融合,生成总的emb
        
    #     if src_key_padding_mask is not None:#[False, False, False,  ...,  True,  True,  True]
    #         total_embs = total_embs * (~src_key_padding_mask).unsqueeze(-1) #pad位置置为0?如果该位置是填充的（<pad>），~src_key_padding_mask的值为False，False在浮点数运算中被视为0，因此total_embs中该位置的值会被置为0
    #     # output = self.mamba_encoder(total_embs) 
    #     output = self.mamba_encoder(total_embs,src_key_padding_mask)  #注意flip操作,应该
    #     return output
         # 使用BiMambaWrapper堆叠
    ...
    def _encode(self, src: Tensor, values: Tensor, src_key_padding_mask: Tensor) -> Tensor:
        src = self.encoder(src)
        values = self.value_encoder(values)
        total_embs = src + values  # 基因和值融合，生成总的emb
        
        if src_key_padding_mask is not None:
            total_embs = total_embs * (~src_key_padding_mask).unsqueeze(-1)
        
        # 手动调用每个 BiMambaWrapper 的 forward 方法
    ...
    def _get_cell_emb_from_layer(self, layer_output: Tensor, weights: Tensor = None) -> Tensor:
        if self.cell_emb_style == "cls":
            ...
            cell_emb = layer_output[:, 1:, :] # 直接返回完整的层输出
        return cell_emb
